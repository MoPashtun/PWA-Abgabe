(function () {
    ...
    function searchForbackCamera() {
        var deferred = new $.Deferred();

        //MediaStreamTrack.getSources seams to be supported only by Chrome
        if (MediaStreamTrack && MediaStreamTrack.getSources) {
            MediaStreamTrack.getSources(function (sources) {
                var backCameraIds = sources.filter(function (source) {
                    return (source.kind === 'video' && source.facing === 'environment');
                }).map(function (source) {
                    return source.id;
                });

                if (backCameraIds.length) {
                    deferred.resolve(backCameraIds[0]);
                } else {
    ...
    function setupVideo(backCameraId) {
        var deferred = new $.Deferred();
        var videoSettings = {
            audio: false,
        ...
        //if back camera is available - use it
        if (backCameraId) {
            videoSettings.video.optional.push({
                sourceId: backCameraId
            });
        }
